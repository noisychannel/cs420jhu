...
class SealedDES implements Runnable {
	...
	//The search range for this parallel decrypting task
	long searchFrom;
	long searchTo;
	//The sealedObj for the parallel decrypting task
	SealedObject parallelSldObj;
	...
	// Constructor: For parallel decrypting, set the maxKey and the sldObj
	public SealedDES(long searchFrom, long searchTo, SealedObject sldObj, int threadId) {
		
		this.searchFrom = searchFrom;
		this.searchTo = searchTo;
		this.parallelSldObj = sldObj;
	...
	// Program demonstrating how to create a random key and then search for the
	// key value.
	...
	public void run() {
		...
		SealedDES deccipher = new SealedDES();
		// Search for the right key
		for (long i = this.searchFrom; i < this.searchTo; i++) {
			// Set the key and decipher the object
			deccipher.setKey(i);
			String decryptstr = deccipher.decrypt(this.parallelSldObj);
			...
			if (i % 100000 == 0) {
				long elapsed = System.currentTimeMillis() - runstart;
				System.out.println("Thread " + this.threadId + " Searched key number " + i + " at "
						+ elapsed + " milliseconds.");
			}
		}
